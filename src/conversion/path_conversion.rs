...
/// reason the delete must refuse. Unlike a write, deleting one stored field
/// needs no value transformation; an ordered candidate plan is safe here
/// because deleting every possible source asserts the same absence.
