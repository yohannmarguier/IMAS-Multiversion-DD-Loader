...
mod tests {
        ...
        for (record, raw, expected) in [
            ...
            match narrow_read_path(resolve(record, raw.as_ptr())) {
                ReadPath::Translated(paths) => {
                    ...
                    assert!(paths.paths.iter().all(|path| path.fidelity == Fidelity::Exact));
                }
    ...
    #[test]
    fn equilibrium_write_and_delete_preserve_shape_refusals_and_delete_boundaries() {
        const ARTIFACT: &str = include_str!("../../docs/3.39.0--4.1.1.xml");
        let record = reverse_record(ARTIFACT);
        let retyped = CString::new("grids_ggd/grid/space/coordinates_type").expect("no interior NUL");
        ...
        let leaf = CString::new("time_slice/global_quantities/beta_tor_norm").expect("no interior NUL");
        ...
        for (raw, expected) in [
            ...
            match narrow_delete_path(&record, raw.as_ptr(), resolve(&record, raw.as_ptr())) {
                DeletePath::Translated(path) => assert_eq!(path.to_str().expect("fixture paths are UTF-8"), expected),
