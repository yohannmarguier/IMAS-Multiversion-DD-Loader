//! Runtime binding to IMAS-Core: how the shim finds `libal`, checks it, and
//! calls into it.
//!
...
//! visible exports are never in the lookup scope and can't capture its
//! outbound calls. See `docs/adr/0001-runtime-binding-not-linking.md`.
//!
//! **ADR 0001 is the only ADR this module enforces, and nothing here makes a
//! conversion decision.** No DD path is read, rewritten or refused, no
//! context record is consulted, and the only versions this module compares
...
//! rest of issue #101's series; this module simply stays on the side of it
...
macro_rules! core_binding {
    (
        ...
        // Crate-visible only so that `forward_status!` can name a field from
        // the module it expands in; nothing outside this module holds a
        // `CoreBinding` or resolves one.
        pub(crate) struct CoreBinding {
            // Kept alive for the process's lifetime: dropping it would unmap
            // the resolved function pointers below. Never read again once
            // resolution succeeds.
            _library: Library,
            $(pub(crate) $bootstrap_field: $bootstrap_type,)*
...
//
// `$crate::core_binding::core` rather than a bare `core()`: `macro_rules!` is
// not hygienic for paths, so the expansion has to name the function from
// wherever it lands.
