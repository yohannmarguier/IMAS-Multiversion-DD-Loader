...
**201 ctest tests** (18 labelled `real-core`; the `IMAS_MVDD_REAL_CORE_TESTS=OFF`
stub-only profile registers 179). None of the C sources here is registered by
...
| `shim/` | 11 C suites driving the public ABI against that stub — 172 tests. |
...
### `write-delete-*` — 36 · `shim/write_delete_conversion_test.c`
...
skip behaviour. A candidate plan writes only its precedence-1 stored path;
non-primary HLI aliases and the DD version stamp refuse before Core. The
