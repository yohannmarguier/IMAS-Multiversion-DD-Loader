...
static void scenario_verbatim_forwarding(void) {
    ...
    /* al_read_data */
    {
        ...
        void *data = NULL;
        int size[1] = {0};
        al_status_t status = al_read_data(2001, "temperature", "time", &data, IMAS_DOUBLE_DATA,
                                           1, size);
        CHECK(status.code == 0);
        CHECK(call_count() == 1);
        CHECK(ctx_id() == 2001);
        ...
        CHECK(strcmp(timebase(), "time") == 0);
        CHECK(datatype() == IMAS_DOUBLE_DATA);
        CHECK(dim() == 1);
        ...
        al_status_t not_found_status = al_read_data(2001, "missing_field", "time", &not_found_data,
                                                      IMAS_DOUBLE_DATA, 1, not_found_size);
    ...
    /* al_write_data */
    {
        ...
        int size[1] = {2};
        al_status_t status =
            al_write_data(2001, "temperature", "time", payload, IMAS_DOUBLE_DATA, 1, size);
        CHECK(status.code == 0);
        CHECK(call_count() == 1);
        CHECK(ctx_id() == 2001);
        CHECK(strcmp(field(), "temperature") == 0);
...
static void scenario_plugin_forwarding(void) {
    ...
    CHECK(last_ctx() == 703);
    CHECK(al_setvalue_parameter_plugin("coefficients", IMAS_INTEGER_DATA, 1, &size, values,
                                       "recording-plugin").code == 0);
    CHECK_PLUGIN_CALL(9, "al_setvalue_parameter_plugin");
    CHECK(strcmp(first_string(), "coefficients") == 0);
    CHECK(strcmp(second_string(), "recording-plugin") == 0);
    CHECK(first_int() == IMAS_INTEGER_DATA && second_int() == 1
    ...
    CHECK(last_ctx() == 707);
    CHECK(al_plugin_read_data(708, "temperature", "time", &read_data, IMAS_DOUBLE_DATA, 1,
                              &size).code == 0);
    CHECK_PLUGIN_CALL(16, "al_plugin_read_data");
    CHECK(last_ctx() == 708 && first_int() == IMAS_DOUBLE_DATA && second_int() == 1);
    ...
    CHECK(size == 4004);
    CHECK(al_plugin_write_data(709, "temperature", "time", &write_data, IMAS_DOUBLE_DATA, 0,
                               NULL).code == 0);
    CHECK_PLUGIN_CALL(17, "al_plugin_write_data");
    CHECK(last_ctx() == 709 && pointer_value() == &write_data);
    CHECK(first_int() == IMAS_DOUBLE_DATA && second_int() == 0);
    CHECK(strcmp(first_string(), "temperature") == 0 && strcmp(second_string(), "time") == 0);
