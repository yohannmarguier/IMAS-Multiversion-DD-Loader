...
static void scenario_relative_field_and_timebase_resolve_through_renamed_child(void) {
    ...
    void *data = NULL;
    int shape[1] = {0};
    CHECK(al_read_data(arraystruct_ctx, "measured", "time", &data, IMAS_DOUBLE_DATA, 1, shape).code == 0);
    CHECK(data != NULL);
...
static void scenario_no_source_returns_null_through_nested_child(void) {
    ...
    void *data = (void *)1;
    int shape[1] = {0};
    CHECK(al_read_data(time_slice_ctx, "boundary/lcfs", "", &data, IMAS_DOUBLE_DATA, 1, shape).code == 0);
    CHECK(data == NULL);
...
static void scenario_sign_flip_applies_through_nested_child(void) {
    ...
    int read_size[1] = {0};
    void *data = NULL;
    CHECK(al_read_data(flux_loop_ctx, "measured", "", &data, IMAS_DOUBLE_DATA, 1, read_size)
              .code == 0);
    CHECK(data != NULL);
...
static void scenario_moved_read_through_nested_child_retains_lossy_verdict_on_root(void) {
    ...
    void *data = NULL;
    int shape[1] = {0};
    CHECK(al_read_data(time_slice_ctx, "boundary_separatrix/gap/r", "", &data, IMAS_DOUBLE_DATA, 1, shape).code
          == 0);
    CHECK(data != NULL);
...
static void scenario_merged_read_through_nested_child_retains_potentially_lossy_verdict(void) {
    ...
    void *data = NULL;
    int shape[1] = {0};
    CHECK(al_read_data(time_slice_ctx, "ggd/b_field_phi", "", &data, IMAS_DOUBLE_DATA, 1, shape).code == 0);
    CHECK(data != NULL);
