...
**Correction (why a restored tree was still rebuilt from scratch).** Fixing the scope was necessary and not sufficient. The first genuinely warm run restored its tree and then rebuilt almost all of it anyway: `al-fortran-sources` regenerated in 5m31s and `al-fortran` recompiled in 12m08s, with only IMAS-Core reused. The cause is not CMake but `actions/checkout`, which writes every file with the checkout's own timestamp — so on every run each HLI source is newer than every object in the restored tree, and make is correct to rebuild. Caching a build tree across runs is therefore only worth anything if the source timestamps are made stable, which this workflow now does by stamping the whole HLI checkout with one fixed mtime.

That stamp is what forces the restore-keys ladder down to a single rung. A constant mtime is safe only while a restored tree was built from exactly the bytes now on disk; reusing a tree across HLI commits would make changed sources look older than stale objects and quietly skip rebuilding them. Keeping the pinned commit in every rung makes that unreachable, and leaves the reuse that matters: Core moving, which is the frequent miss and rebuilds in about thirty seconds.
