...
2. **Each root context carries a loss log.** For every non-exact read *or write* the shim appends the DD path *as the HLI asked for it*, the fidelity verdict (ADR 0008), and which **operation** produced the entry. The operation field is deliberately not called a direction: `conversion_map::Direction` already means which side of the version pair supplies the path, and reusing the word for read-versus-write would collide with an existing term meaning something else. A read under an arraystruct context follows the parent links up **at record time** and appends to the root context's log; a query resolves up to the root the same way. A query on any context under one IDS therefore returns that whole IDS read, and the shim never scans for children — ADR 0003 forbids a read-time hierarchy walk and this respects it.
