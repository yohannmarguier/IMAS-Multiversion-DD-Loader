...
1. **A read never reports loss through `al_status_t`.** On a successful read the code stays `0` and the message is untouched, whatever the fidelity verdict. The code channel carries refusals only. The shim adds no cost, no accumulator and no log rendering to the read path.

2. **Each root context carries a loss log.** For every non-exact read the shim appends the DD path *as the HLI asked for it* and the fidelity verdict (ADR 0008). A read under an arraystruct context follows the parent links up **at record time** and appends to the root context's log; a query resolves up to the root the same way. A query on any context under one IDS therefore returns that whole IDS read, and the shim never scans for children — ADR 0003 forbids a read-time hierarchy walk and this respects it.
...
6. **Two new exports, and nothing is allocated.** `imas_mvdd_context_loss_count(ctx, *n)` and `imas_mvdd_context_loss_at(ctx, i, path_buf, buf_len, *verdict)`. No struct layout is published and no memory crosses the boundary, so ADR 0006's finding that the mirrored ABI has no free seam continues to hold. With `imas_mvdd_set_hli_dd_version` this makes three shim-owned exports, all on ADR 0005's drift list.
...
- **Shim-owned exports go from one to three.** The export-drift check's owned list grows by two.
