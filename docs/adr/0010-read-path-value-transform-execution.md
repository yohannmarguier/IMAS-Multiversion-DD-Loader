...
Every shim-originated conversion refusal returns the shim-owned public code `IMAS_MVDD_CONVERSION_ERROR` (`-1000`). Its `al_status_t.message` starts with `IMAS-MVDD:` and states the reason, DD path, HLI DD version, and stored DD version within `MAX_ERR_MSG_LEN`. Unit redefinitions, unsupported data types, and invalid shapes use this code. A refusal leaves caller-provided `data` and `size` storage and any IMAS-Core buffer unchanged.
