...
- Because there is no link-time record of which IMAS-Core the shim was built against, a runtime version check is required: the shim must ask IMAS-Core its own library version (not a DD version) on first resolution and compare it against the version it was built against, so a mismatch fails through the ABI's status channel instead of calling through the wrong signatures.
