...
- No linker validates that the shim's hand-written signatures agree with IMAS-Core's real ones. A separate CI check must therefore compile a translation unit against IMAS-Core's real header and assert struct layout and function addresses. This is required follow-up work; the current `abi-smoke` test uses the shim's generated header and does not provide that validation.
